...
#include "MergingRAVENNACommon.h"
#include "MTConvert.h"
                                        ...
                                        struct snd_pcm_hw_params *params)
{
    ...
    if (runtime->dma_area == NULL || runtime->dma_bytes < bufferBytes)
    {
      if (runtime->dma_area != NULL)
	vfree(runtime->dma_area);

      runtime->dma_area = vzalloc(bufferBytes);
      if (!runtime->dma_area)
	return -ENOMEM;

      runtime->dma_bytes = bufferBytes;
    }
...
static struct page *mr_alsa_audio_pcm_get_page(struct snd_pcm_substream *sub,
...
static struct snd_pcm_ops mr_alsa_audio_pcm_playback_ops = {
    ...
    .page =     mr_alsa_audio_pcm_get_page,
};
